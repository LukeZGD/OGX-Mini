...
const tusb_control_request_t PS3V2Host::RUMBLE_REQUEST = {
    .bmRequestType = 0x21,
    .bRequest = 0x09, // SET_REPORT
    .wValue = 0x0201,
    .wIndex = 0x0000,
    .wLength = sizeof(PS3V2::OutReport)};

void PS3V2Host::initialize(Gamepad &gamepad, uint8_t address, uint8_t instance,
                           const uint8_t *report_desc, uint16_t desc_len) {
  gamepad.set_analog_host(true);

  std::memcpy(
      reinterpret_cast<uint8_t *>(&out_report_), PS3V2::DEFAULT_OUT_REPORT,
      std::min(sizeof(PS3V2::OutReport), sizeof(PS3V2::DEFAULT_OUT_REPORT)));
  ...
  // Initial Request: GET_REPORT Feature 0xF2
  ...
  send_control_xfer(address, &init_request, init_state_.init_buffer.data(),
                    get_report_complete_cb,
                    reinterpret_cast<uintptr_t>(&init_state_));

  tuh_hid_receive_report(address, instance);
}

bool PS3V2Host::send_control_xfer(uint8_t dev_addr,
...
void PS3V2Host::get_report_complete_cb(tuh_xfer_s *xfer) {
  ...
  case InitStage::RESP1:
    // Came from F2, now do another GET_REPORT F2
    ...
    request = {.bmRequestType = 0xA1,
               ...
               .wIndex = 0x0000,
               .wLength = 17};
    send_control_xfer(init_state->dev_addr, &request,
                      init_state->init_buffer.data(), get_report_complete_cb,
                      xfer->user_data);
    break;

  case InitStage::RESP2:
    // Came from 2nd F2, now do GET_REPORT F2 (length 8) - likely checking
    // subset
  ...
  case InitStage::RESP3:
    // Came from 3rd F2, now Send MAGIC PACKET F4
    // This is the critical step for original controllers
    init_state->stage = InitStage::RESP4;

    // Prepare buffer with F4 data
    std::memcpy(init_state->init_buffer.data(), PS3V2::OUTPUT_0xF4,
                sizeof(PS3V2::OUTPUT_0xF4));

    request = {.bmRequestType = 0x21, // Host-to-Device | Class | Interface
               .bRequest = 0x09,      // SET_REPORT
               .wValue = (HID_REPORT_TYPE_FEATURE << 8) | 0xF4,
               .wIndex = 0x0000,
               .wLength = sizeof(PS3V2::OUTPUT_0xF4)};
  ...
  case InitStage::RESP4:
    // Came from F4, now finish and enable rumble
    init_state->stage = InitStage::DONE;
    init_state->reports_enabled = true;
    send_control_xfer(init_state->dev_addr, &PS3V2Host::RUMBLE_REQUEST,
                      reinterpret_cast<uint8_t *>(init_state->out_report),
                      nullptr, 0);
    break;
...
void PS3V2Host::process_report(Gamepad &gamepad, uint8_t address,
                               uint8_t instance, const uint8_t *report,
                               uint16_t len) {
  const PS3V2::InReport *in_report =
      reinterpret_cast<const PS3V2::InReport *>(report);
  if (std::memcmp(
          &prev_in_report_, in_report,
          std::min(static_cast<size_t>(len), static_cast<size_t>(26))) == 0) {
    tuh_hid_receive_report(address, instance);
    return;
  }
  ...
  if (in_report->buttons[0] & PS3V2::Buttons0::DPAD_UP)
    gp_in.dpad |= gamepad.MAP_DPAD_UP;
  if (in_report->buttons[0] & PS3V2::Buttons0::DPAD_DOWN)
    gp_in.dpad |= gamepad.MAP_DPAD_DOWN;
  if (in_report->buttons[0] & PS3V2::Buttons0::DPAD_LEFT)
    gp_in.dpad |= gamepad.MAP_DPAD_LEFT;
  if (in_report->buttons[0] & PS3V2::Buttons0::DPAD_RIGHT)
    gp_in.dpad |= gamepad.MAP_DPAD_RIGHT;

  if (in_report->buttons[0] & PS3V2::Buttons0::SELECT)
    gp_in.buttons |= gamepad.MAP_BUTTON_BACK;
  if (in_report->buttons[0] & PS3V2::Buttons0::START)
    gp_in.buttons |= gamepad.MAP_BUTTON_START;
  if (in_report->buttons[0] & PS3V2::Buttons0::L3)
    gp_in.buttons |= gamepad.MAP_BUTTON_L3;
  if (in_report->buttons[0] & PS3V2::Buttons0::R3)
    gp_in.buttons |= gamepad.MAP_BUTTON_R3;
  if (in_report->buttons[1] & PS3V2::Buttons1::L1)
    gp_in.buttons |= gamepad.MAP_BUTTON_LB;
  if (in_report->buttons[1] & PS3V2::Buttons1::R1)
    gp_in.buttons |= gamepad.MAP_BUTTON_RB;
  if (in_report->buttons[1] & PS3V2::Buttons1::TRIANGLE)
    gp_in.buttons |= gamepad.MAP_BUTTON_Y;
  if (in_report->buttons[1] & PS3V2::Buttons1::CIRCLE)
    gp_in.buttons |= gamepad.MAP_BUTTON_B;
  if (in_report->buttons[1] & PS3V2::Buttons1::CROSS)
    gp_in.buttons |= gamepad.MAP_BUTTON_A;
  if (in_report->buttons[1] & PS3V2::Buttons1::SQUARE)
    gp_in.buttons |= gamepad.MAP_BUTTON_X;
  if (in_report->buttons[2] & PS3V2::Buttons2::SYS)
  ...
  std::tie(gp_in.joystick_lx, gp_in.joystick_ly) =
      gamepad.scale_joystick_l(in_report->joystick_lx, in_report->joystick_ly);
  std::tie(gp_in.joystick_rx, gp_in.joystick_ry) =
      gamepad.scale_joystick_r(in_report->joystick_rx, in_report->joystick_ry);

  gamepad.set_pad_in(gp_in);

  tuh_hid_receive_report(address, instance);
  std::memcpy(&prev_in_report_, in_report, sizeof(PS3V2::InReport));
}

bool PS3V2Host::send_feedback(Gamepad &gamepad, uint8_t address,
                              uint8_t instance) {
  ...
  if (init_state_.reports_enabled && current_ms - last_rumble_ms >= 300) {
    ...
    return send_control_xfer(address, &PS3V2Host::RUMBLE_REQUEST,
                             reinterpret_cast<uint8_t *>(&out_report_), nullptr,
                             0);
  }
  return true;
}
